...
//=======================================================
// Temporary file
$tmpfname = tempnam(sys_get_temp_dir(), 'wildBobcat_kmz_');
...
switch ($_GET["mask"]) {
	case "land":
		$mask_name = "Land mask";
		$mask_file = "mask_land.png";
		$mask_toAdd = True;
		break;
	case "ocean":
		$mask_name = "Ocean mask";
		$mask_file = "mask_ocean.png";
		$mask_toAdd = True;
		break;
	default:
		$mask_name = "none";
...
$kml_logo = '
      <ScreenOverlay>
              ...
              <Icon><href>legend_bg.png</href></Icon>
...
if ($zip->open($tmpfname)) {
    ...
    $zip->addFromString("wildBobcat.kml", $kml_new);

    // add logo file
    $zip->addFile('legend_bg.png');
    $zip->close();
